...
export default {
  ...
  computed: {
    ...
    watchConfig: function () {
      ...
      let watchConfig = Vue.util.extend({},this.config, raw);
      watchConfig.type = 'watcher';
    ...
    existingTradebot: function () {
      return _.find(this.gekkos, (g) => {
        if (
          g.logType === 'tradebot' &&
          g.config.watch.exchange === this.exchange
        ) {
          return true;
        }
  ...
  methods: {
    ...
    start: function () {
      ...
      //
      // however if the user selected type "watcher"
      // the second part won't be created
      if (this.config.type === 'watcher') {
